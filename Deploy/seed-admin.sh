...
# -----------------------------------------------------------------------------
# Provision the SINGLE application account (registration is disabled in this
# deployment). Runs the compiled seed-user CLI INSIDE the active color's server
# container, which already holds DATABASE_URL + full config on km-internal (km-db
# is not published to the host, by design).
#
...
# Re-running is idempotent (ON CONFLICT (email) DO NOTHING) — it will NOT rotate
...
main() {
    ...
    # Email/display are NOT secret → passed via -e. The password is piped on STDIN
    # and read into an env var set INSIDE the container, so it is never on a command
    # line. seed-user.js reads SEED_USER_* from its environment.
    printf '%s\n' "$pw" | docker exec -i \
