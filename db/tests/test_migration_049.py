...
WHY THIS FILE EXISTS:
    049 ALTERS AN EXISTING TABLE that already carries user data: it renames
    `entry_id` -> `vocab_entry_id`, drops its NOT NULL, adds the grammar/hanja
    target columns, swaps the two-column UNIQUE constraint for per-target
    partial UNIQUE indexes, and installs the exactly-one-target CHECK. The
    synthetic harness tests (test_migrations.py) and the foundation
    round-trips (test_migrations_real.py) cannot prove that pre-049 vocab
    memberships survive the reshape or that the new invariants hold on a real
    chain. These tests apply the REAL migration chain against a Postgres-16
    testcontainer via `migrate.main()`, seed rows in the pre-049 shape, and
    assert the reshape — and its best-effort reverse — on actual data.

SCOPE:
    - up: existing vocab rows survive the rename with values in place; the
      XOR CHECK rejects two-target and zero-target rows; per-target partial
      UNIQUE indexes enforce one membership per (list, target) while allowing
      the same target in different lists and different types in one list;
      FK postures asserted from pg_constraint (vocab RESTRICT kept from 012;
      kgiu/hanja CASCADE per the F-048 spec); the 012 UNIQUE constraint and
      the missing-index gap are gone.
    - down: grammar/hanja memberships (no pre-049 representation) are
      removed; vocab memberships round-trip losslessly into the restored
      NOT NULL `entry_id`; the 012 UNIQUE constraint returns; re-up is clean.
...
NB: unlike 046's seed stage (which could stop at 044), the pre-049 target is
    047, which is PAST the destructive 045 — so every `up` here, including
    the seed-stage one, passes --allow-destructive.
"""
...
# The seed target: the last migration before 049 (048 does not exist), i.e.
# the pre-049 vocab-only shape of vocab_list_entries.
PRE_049 = "047"
...
def test_049_up_preserves_vocab_rows_and_enforces_xor(env, dsn: str, full_dir) -> None:
    """Seed pre-049 vocab-only memberships, apply 049, and assert the reshape:
    values moved by RENAME (not copied), new columns NULL, XOR CHECK and
    per-target partial UNIQUE indexes live, FK postures correct."""
    # --allow-destructive: the chain to 047 traverses 045 (hygiene_cleanup,
    ...
    with psycopg.connect(dsn, autocommit=True) as conn:
        ...
        assert "entry_id" in cols and "vocab_entry_id" not in cols
    ...
    with psycopg.connect(dsn, autocommit=True, row_factory=dict_row) as conn:
        # Columns reshaped: rename + two new targets, vocab now nullable.
        cols = _columns(conn, "vocab_list_entries")
        assert "entry_id" not in cols
        assert {"vocab_entry_id", "kgiu_entry_id", "hanja_character_id"} <= cols
        assert _is_nullable(conn, "vocab_list_entries", "vocab_entry_id")

        # Existing rows survived as vocab memberships, values + positions intact.
        with conn.cursor() as cur:
            cur.execute(
                """
                SELECT vocab_entry_id, kgiu_entry_id, hanja_character_id, position
                  FROM vocab_list_entries WHERE list_id = %s ORDER BY position
                """,
                (lst,),
            )
            rows = cur.fetchall()
        assert [(r["vocab_entry_id"], r["position"]) for r in rows] == [
            (v1, 0),
            (v2, 1),
        ]
        assert all(
        ...
        # Constraint/index inventory: old UNIQUE gone, new partial uniques +
        # entry-column indexes present.
        con_names = _constraint_names(conn, "vocab_list_entries")
        assert "uq_vocab_list_entries_list_entry" not in con_names
        assert "ck_vocab_list_entries_target_xor" in con_names
        idx = _index_names(conn, "vocab_list_entries")
        assert {
            "uq_vocab_list_entries_list_vocab",
            "uq_vocab_list_entries_list_kgiu",
            "uq_vocab_list_entries_list_hanja",
            "ix_vocab_list_entries_vocab_entry",
        ...
        # RESTRICT ('r'); the new targets CASCADE ('c') per the F-048 spec.
        with conn.cursor() as cur:
            cur.execute(
                """
        ...
        assert fk["fk_vocab_list_entries_vocab_entry"] == "r"
        ...
        assert exc.value.diag.constraint_name == "uq_vocab_list_entries_list_vocab"
        with pytest.raises(psycopg.errors.UniqueViolation) as exc:
            with conn.cursor() as cur:
                cur.execute(
                    """
...
# ---------------------------------------------------------------------------
# 2. DOWN — best-effort reverse: grammar/hanja memberships removed, vocab
#    rows round-trip into the restored 012 shape; then a clean re-up
# ---------------------------------------------------------------------------
...
def test_049_down_drops_multitype_rows_and_restores_012_shape(
    ...
    # --allow-destructive on the down is not required by the gate (049.down's
    # data loss is via DELETE + DROP COLUMN, which DESTRUCTIVE_PATTERNS does
    # not match — same caveat as 046.down); passed anyway to match the
    # documented rollback posture: rolling back 049 deliberately discards
    # grammar/hanja memberships.
    rc = migrate.main(
    ...
    with psycopg.connect(dsn, autocommit=True, row_factory=dict_row) as conn:
        # Schema restored to the 012 shape.
        cols = _columns(conn, "vocab_list_entries")
    ...
    with psycopg.connect(dsn, autocommit=True, row_factory=dict_row) as conn:
        with conn.cursor() as cur:
            ...
            rows = cur.fetchall()
        assert len(rows) == 1
        assert rows[0]["vocab_entry_id"] == v1
