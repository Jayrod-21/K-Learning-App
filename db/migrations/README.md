...
| 070 | `vocab_cards_source_upload` | F-199 | Per-user user-saved upload provenance: nullable `source_upload_id` FK on `vocab_cards` (named FK → `book_uploads`, `ON DELETE SET NULL`, partial index — mirrors 068's grammar column) + an ownership-guarded, fill-only backfill from the shared `vocab_entries.source_upload_id` (copied only where `book_uploads.user_id = vocab_cards.user_id`; mis-attributed cross-user tags are deliberately dropped — the exact F-199 bug). `POST /vocab/mine` stops writing the shared row's tag in the same change; `vocab_entries.source_upload_id` is F-108 extracted-corpus provenance only from here on. Up marked `-- migrate: non-destructive` (ADD COLUMN + fill-only UPDATE); down `DROP COLUMN` marked destructive |
| 082 | `fix_topik_222_stem` | B-031 | One-row content fix (071–081 predate this row and are documented in their own `.up.sql` headers): the 83rd TOPIK I 읽기 Q42 stem (id 222 in prod) gains the photo description its transcription dropped — the SNS post's photo shows 수미 at '제주공항', which is what makes option ① true ("맞지 않는 것" task, answer ④). Option text matches the official scan verbatim and is untouched. Content-addressed UPDATE (keys on the exact old stem + option ① text, not on an id) → idempotent, no-op on fresh/already-fixed DBs; up and down both `-- migrate: non-destructive` (exact round-trip) |
| 084 | `ability_evidence_view` | F-212 P1 | Read-only `ability_evidence` VIEW (083 predates this row, documented in its own `.up.sql` header): `UNION ALL` over the six append-only graded logs — topik_responses (+items/tests), card_reviews (+vocab_cards, hanja cards excluded — leg 5 owns the cardReview.ts dual-write), scored grammar_drill_attempts, writing_attempts (+prompts), hanja_attempts, answered diagnostic_responses (+runs) — each leg emitting the same 13 normalized columns (user/dimension/source/item_key/occurred_at + raw outcome + raw difficulty signals). Zero write-paths touched; outcome/difficulty math lives in `server/src/services/ability/`. Up `-- migrate: non-destructive` (CREATE OR REPLACE VIEW); down `DROP VIEW` marked destructive by convention though no data is lost (pure derivation, re-up rebuilds it) |
| 085 | `topik_item_images` | F-120 P1 | TOPIK question images, plumbing only: nullable `topik_items.image_ref` TEXT — the corpus-RELATIVE key of the question's cropped exam figure under CORPUS_IMAGE_DIR (035/078's relative-key contract, per-item this time). Ships EMPTY (no backfill — every row NULL until `tools/ingest/loaders/load_topik_image.py` runs); served by `GET /topik/image/:testNumber/:level/:itemNumber`. No index (only SELECTed via the existing item keys), no CHECK (loader parse-time validation + the serving route's containment guard own the relative-key contract). Up `-- migrate: non-destructive` (ADD COLUMN IF NOT EXISTS); down `DROP COLUMN` marked destructive (recoverable from the extraction manifests, 078's posture) |
| 086 | `reading_questions` | F-205 P1 | AI-generated MC reading-comprehension checks: `claude_route` gains `'reading_comprehension'` (ADD VALUE, 053/057/083's posture) + `reading_questions` — one row per (chapter, question_number): Korean `question_text`, `options` JSONB (exactly 4 `{text, correct}` — CHECK pins array-ness + arity; exactly-one-correct is the proxy Zod refine's contract), bilingual `explanation`, `kind` CHECK (`'comprehension'` only — forward-compat seam), `model` provenance. No `user_id` — access via the parent chapter (`reading_passages`' posture); CASCADEs from `reading_chapters`. Generate-once structural via `UNIQUE (chapter_id, question_number)`. Ships EMPTY (no chapter has questions until `POST /reading/chapters/:chapterId/questions/generate` runs). Up `-- migrate: non-destructive`; down `DROP TABLE` marked destructive (re-derivable — a paid Claude call per chapter; the enum value stays, Postgres cannot drop enum values) |
| 087 | `diagnostic_hanja_section` | diagnostic-upgrade Phase A | Widens `ck_diagnostic_responses_section` (014) to add `'hanja'` — the fifth, COVERAGE-ONLY diagnostic dimension: a plain 4-choice MC item (reading-of-character or meaning-of-character) built from the existing `hanja_characters` corpus (016 — L2/L3 only), no Claude call. Coverage-only = scored (own `dimensionStats` entry) but excluded from the run's global θ ladder (`nextTheta` is never called for a hanja response — the corpus's L3 ceiling must not drag an advanced learner's placement down). `source_kind` is untouched — hanja reuses `'generated'` (already valid) rather than adding a `'corpus'` value, since `section` alone discriminates it. Pairs with `RUBRIC_VERSION` v1.2.0→v1.3.0 (`server/src/services/diagnostic/scoring.ts`) and `SEED_THETA`/`stepForAnswer` retune (start-easy adaptive ramp, same Phase A build, no schema surface). Up `-- migrate: non-destructive` (CHECK widen only); down `-- migrate: destructive` (re-narrowing CheckViolations if a hanja-tagged row exists post-087, mirrors 056's writing_attempts.rubric rollback posture) |
| 088 | `diagnostic_writing_section` | diagnostic-upgrade Phase B | Widens `ck_diagnostic_responses_section` (014, 087) to add `'writing'` — the sixth diagnostic dimension, and unlike hanja a FULL LEVELED one: a writing response DOES bump the run's global θ ladder and DOES consume a step-ordinal slot (the `/answer` handler's existing `section <> 'hanja'` guard already generalizes, no new θ-gating code needed). Built + graded by the EXISTING `generateGrammarDrill`/`scoreGrammarDrill` Claude pipeline (Pass 9's grammar-production-drill route, reused — no new Claude route), seeded from a `kgiu_entries` grammar pattern; verdict `{excellent,good}`→correct, `{needs_work,incorrect}`→wrong. `source_kind` untouched — writing reuses `'generated'`. Pairs with `RUBRIC_VERSION` v1.3.0→v1.4.0 and `DIMENSION_ORDER` gaining `'writing'` as its 6th member (`server/src/services/diagnostic/scoring.ts`); `writing` is deliberately NOT added to `CORE_DIMENSION_ORDER` (the F-212 estimator keeps it opt-in via `includeWriting`, unchanged by this migration). Up `-- migrate: non-destructive` (CHECK widen only); down `-- migrate: destructive` (re-narrowing CheckViolations if a writing-tagged row exists post-088, mirrors 087's hanja rollback posture exactly) |
| 089 | `diagnostic_dimension_estimates` | diagnostic-upgrade Phase C | Adds `diagnostic_runs.dimension_estimates JSONB NOT NULL DEFAULT '{}'::jsonb` (CHECK `jsonb_typeof(...) = 'object'`) — a per-run, per-dimension adaptive θ SERVING CACHE keyed dimension→theta, so each leveled dimension (reading/listening/vocab/grammar/writing) climbs its OWN staircase instead of sharing one global ladder; warm-started from the run's existing `ability_estimate` on a dimension's first item, then stepped independently thereafter (`server/src/routes/diagnostic.ts` serveNextItem / POST `/:runId/answer`). `hanja` never gets a key — it stays coverage-only, served/scored off the global θ unchanged. `ability_estimate` itself is UNTOUCHED in meaning: still the overall θ, still the warm-start source, still the sole F-212 evidence input. Pairs with `RUBRIC_VERSION` v1.4.0→v1.5.0 (a leveled dimension's snapshot estimate becomes its final ladder θ, not the old mean-difficulty+p heuristic) and `WEIGHTS`/`SCHEDULE` (reading/listening/vocab/grammar=6, hanja=4, writing=2 → 30 items). Up `-- migrate: non-destructive` (ADD COLUMN with a default backfill, CHECK add); down `-- migrate: destructive` (DROP COLUMN — discards in-progress runs' live per-section cache; a finished run's real estimates already live in `diagnostic_snapshots`, untouched; see the down.sql header for why a losing run degrades gracefully rather than corrupting) |
| 090 | `audio_transcription_jobs_running_index` | audit Phase 0.1 | Adds a PARTIAL INDEX `ix_audio_transcription_jobs_running` on `audio_transcription_jobs (started_at) WHERE status = 'running'`, matching the exact predicate the A1 worker's stale-job reaper runs on every claim poll (`tools/audio_stt/worker.py` `reap_stale`). Without it the reaper was a Seq Scan — measured live at 441,744,717 lifetime tuples read to find zero stale rows. Up `-- migrate: non-destructive` (additive index only); down `-- migrate: destructive` not needed — `DROP INDEX IF EXISTS` is pure subtraction, no data loss. No `CONCURRENTLY` (forbidden inside the runner's transaction, ADR-013); table is small (~982 rows), so a plain `CREATE INDEX`'s brief write lock is acceptable |
