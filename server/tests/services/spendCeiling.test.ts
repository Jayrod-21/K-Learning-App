 ...
 *     story_image_jobs(done); excludes rows outside the window; excludes a
 *     non-done job even when it carries a (direct-SQL-seeded) cost value.
...
describe('getGlobalSpendUsdSince', () => {
  it('sums cost_estimate_usd across claude_usage + story_audio_jobs(done) + story_image_jobs(done)', async () => {
    ...
    expect(result.total).toBeCloseTo(2.15, 6);
  });
...
describe('getSpendCeilingStatus', () => {
  ...
  it('enabled: exact breakdown + remaining_usd = ceiling - spent', async () => {
    ...
    expect(status.spent_usd).toEqual({ total: expect.closeTo(6, 6), claude: expect.closeTo(3, 6), tts: expect.closeTo(1, 6), images: expect.closeTo(2, 6) });
