...
describe('runStoryAudioTick', () => {
  ...
  it('Phase 1.3: the idle color reaps stale jobs but never claims pending ones', async () => {
    ...
    const pendingId = await seedStoryAudioJob(pg.pool, userId, pendingStory, {
      status: 'pending',
    });
    setTtsProvider(okProvider('unused — must never be called'));

    const otherColorFile = path.join(
      await mkdtemp(path.join(os.tmpdir(), 'km-story-audio-active-color-')),
      'active-color',
    );
    ...
    const rows = await pg.pool.query<{ id: string; status: string }>(
      `SELECT id::text AS id, status FROM story_audio_jobs ORDER BY id`,
    );
