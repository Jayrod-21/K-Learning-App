 ...
 * minutes cold in CI — pip install kiwipiepy + a ~125MB model, image ~486MB)
 * + a model load (~seconds for the base model). That is the price of the only
 * test in the repo that exercises the real contract.
 *
 * GATED BEHIND `KIWI_LIVE` (opt-in): because of that cost, this suite is
 * SKIPPED in the default `npm test` run — otherwise every server CI run and
 * every local inner-loop `npm test` would pay a multi-minute docker build plus
 * a PyPI-network flake surface. It still runs where it matters:
 *   - Deploy/local-test.sh sets KIWI_LIVE=1 for its server suite (M has a
 *     warm docker layer cache), so the pre-deploy gate catches kiwi drift;
 *   - ci.yml's docker-build job re-runs just this file with KIWI_LIVE=1 and
 *     KIWI_LIVE_IMAGE=<the kiwi image it just built>, so CI catches drift
 *     without taxing the main Server job.
 * Run it by hand with:
 *   KIWI_LIVE=1 npx vitest run tests/services/kiwi.live.test.ts
 *
 * `KIWI_LIVE_IMAGE` (optional): name/tag of a PRE-BUILT km-kiwi image to boot
 * instead of building from services/kiwi/Dockerfile. Used by CI to reuse the
 * docker-build job's image; when unset (the local default) the test builds
 * from the Dockerfile so it always exercises the CURRENT service source.
 *
...
// Opt-in gate (see header): the default suite must never pay the image build.
// NOTE: the hooks live INSIDE the gated describe — a file-level beforeAll
// would still build the image even with every test skipped.
const KIWI_LIVE = Boolean(process.env.KIWI_LIVE);

let container: StartedTestContainer;

describe.skipIf(!KIWI_LIVE)(
  'lemmatize ↔ real km-kiwi (schema-drift smoke, F-218)',
  () => {
    beforeAll(async () => {
      // CI reuse path: boot a pre-built image by name when KIWI_LIVE_IMAGE is
      // set; otherwise (local default) build from the service's Dockerfile so
      // the test always exercises the current source.
      const image = process.env.KIWI_LIVE_IMAGE
        ? new GenericContainer(process.env.KIWI_LIVE_IMAGE)
        : await GenericContainer.fromDockerfile(KIWI_CONTEXT).build();
      container = await image
        ...
        .withWaitStrategy(
          Wait.forHttp('/health', KIWI_PORT).forResponsePredicate((body) => {
            try {
              return (
                (JSON.parse(body) as { model_loaded?: boolean }).model_loaded === true
              );
            } catch {
              return false;
            }
