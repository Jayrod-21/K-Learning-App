...
describe('claude_route enum ⇄ RouteName drift guard', () => {
  ...
  it("057's 'translate_passage' value is present in the migrated enum (F-116)", async () => {
    ...
    // ever reverted alongside a bad migration revert, this still fails
  ...
  it("104's 'generate_listening_item' value is present in the migrated enum (F-220 slice 3)", async () => {
    // Mirrors the 053/055/057/102 pins above: an explicit probe for
    // migration 104's ADD VALUE, independent of ROUTE_NAMES — if the
    ...
    // this file.
    const { rows } = await pg.pool.query<{ present: boolean }>(
      `SELECT 'generate_listening_item' = ANY(
                ARRAY(SELECT e::text FROM unnest(enum_range(NULL::claude_route)) AS e)
              ) AS present`,
    );
    expect(rows[0]?.present).toBe(true);
