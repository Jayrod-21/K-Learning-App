...
- 🟡 **F-210** P2 — story TTS audio (ElevenLabs) + read-along highlighting. **Code COMPLETE + full 4-phase fixpass, PR #181 (ships DORMANT).** Migration 081, on-demand voice-once, in-Node job runner, `ttsConfigured` capability gate. **DORMANT until `ELEVENLABS_API_KEY` is set** (a friend's ElevenLabs account) — going live = env edit + redeploy, no code change. v1 = narrator single-voice; multi-voice `turns` groundwork landed for v2. The copyright-clean answer to B-025/B-026.
...
- **F-210** 🔴→🟡 — story TTS audio (ElevenLabs) + read-along highlighting. Code COMPLETE + full 4-phase fixpass; PR **#181** (ships DORMANT). On-demand "Generate audio" → voice-once (cached) via ElevenLabs → real `<audio>` + sentence-sync highlighting from timestamps; v1 narrator single-voice on flat `body_ko`, multi-voice `turns` groundwork additive/latent for v2. **Migration 081** (`generated_stories.turns`, `audio_sources` widened to `generated_story` + owner FK, `story_audio_jobs`). Reuses the existing audio blob-store/tracks/segments + `/audio/tracks/:id/stream`; in-Node job runner (GPU worker mounts audio read-only), advisory-locked, per-user daily cap, no half-state. **DORMANT until `ELEVENLABS_API_KEY` set** (friend's account) — key `.optional()` everywhere, `${VAR:-}` passthrough in both color composes + `.env.example`, `ttsConfigured` envelope flag hides the audio card until configured; go-live = env edit + redeploy, no code change. `SECURITY.md §21`. Addresses B-025/B-026 (copyright-clean listening). NOT yet merged/deployed — **active stays green `tog-28a45d4`**.
...
| F-210 | Feature | 🟡 | P2 | API (BACKEND, DATABASE, UI) | Story TTS audio (ElevenLabs) + read-along highlighting; migration 081, voice-once, dormant until key. Code complete + fixpassed, PR #181 |
...
- **Status:** 🔴 open · **Priority:** P3 · **Category:** API (BACKEND, UI) · **half of the F-216 epic** (read F-216's three shared decisions first)
...
- **Status:** 🔴 open (umbrella; text half done) · **Priority:** P2 · **Category:** API (BACKEND, DATABASE, UI) · epic
